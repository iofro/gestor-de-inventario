...
# Directory to store PDF/JSON files whose venta has been removed
ORPHAN_INVOICES_DIR = os.path.join(os.path.dirname(__file__), "facturas_huerfanas")
...
class FacturacionTab(QWidget):
    ...
    def _get_invoices_from_db(self):
        ...
        for row in self.manager.db.cursor.execute("SELECT venta_id, ruta FROM facturas_pdf"):
            ...
            # If the sale no longer exists, move files to orphan dir and
            # remove the DB record to avoid showing "Sin venta"
            if venta is None:
                for f in (pdf, js):
                    if f and os.path.exists(f):
                        os.makedirs(ORPHAN_INVOICES_DIR, exist_ok=True)
                        try:
                            shutil.move(f, os.path.join(ORPHAN_INVOICES_DIR, os.path.basename(f)))
                        except Exception:
                            pass
