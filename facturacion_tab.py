...
# Normalized labels used across the UI. These values intentionally match the
# options displayed in ``self.tipo_filter`` so that filtering can rely on the
# canonical representation even when the underlying source (database, folder
# structure or orphan files) stores a different variant.
...
class FacturacionTab(QWidget):
    ...
    def _get_invoices_from_db(self):
        ...
        for rec in combined_records:
            ...
            tipo_desc = self._infer_tipo_from_name(base_name, tipo_desc or doc_tipo)
            row = {
                "row_type": row_type,
                "id": rec.get("id"),
                "venta_id": venta_id,
    ...
    def _normalize_tipo_key(value: str | None) -> str:
        if not value:
            return ""
        key = str(value).strip()
        # Replace separators and split camel case before normalizing.
    ...
    @staticmethod
    def _canonical_tipo_label(value: str | None) -> str | None:
        key = FacturacionTab._normalize_tipo_key(value)
        if not key:
            return None
        return CANONICAL_TIPO_LABELS.get(key)

    def _infer_tipo_from_name(self, base_name: str | None, fallback: str | None = None) -> str | None:
        # Prefer the suffix embedded in the JSON/PDF file name.
        suffix = None
        if base_name:
            # ``base_name`` may already be the filename without extension, but
            # make sure we strip any directory information just in case.
            cleaned = os.path.splitext(os.path.basename(base_name))[0]
            match = DOC_PATTERN.match(cleaned)
            if match:
                ...
                if parts:
                    suffix = parts[-1]
        if suffix:
            inferred = self._canonical_tipo_label(suffix)
            if inferred:
                return inferred

        fallback_canonical = self._canonical_tipo_label(fallback)
