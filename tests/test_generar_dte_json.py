...
def test_generar_dte_json_precios_incluyen_iva_unitario(tmp_path):
    ...
    assert money(D(str(item["ventaGravada"])) - D(str(item["ivaItem"]))) == D("11.50")
...
def test_generar_dte_json_precios_incluyen_iva_multiple_cant(tmp_path):
    ...
    assert money(D(str(item["ventaGravada"])) - D(str(item["ivaItem"]))) == D("9.73")
...
def test_generar_dte_json_cf_descuento_cant(tmp_path, descuento):
    ...
    base = money(line_total / Decimal("1.13"))
    iva_item = money(line_total - base)
    ...
    assert Decimal(str(res["subTotalVentas"])) == base
