...
def test_generar_nce_detalle_parcial(monkeypatch):
    _prep(monkeypatch)
    db = create_db()
    db.add_vendedor("V1")
    vid = db.cursor.lastrowid
    ...
    data = generar_nce_desde_dte(db, dte_origen, None, detalles=detalles, motivo="Dev")
    resumen = data["resumen"]
    expected_iva = Decimal("2") * Decimal("0.13")
    assert resumen["tributos"][0]["valor"] == expected_iva
    assert resumen["montoTotalOperacion"] == Decimal("2") + expected_iva
